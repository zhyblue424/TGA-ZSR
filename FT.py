...
def parse_option():
    ...
    # optimization
    parser.add_argument('--Method', type=str, default='FT-clean')
    ...
    parser.add_argument('--dataset', type=str, default='tinyImageNet',
                        choices=['cifar100', 'ImageNet', 'cifar10', 'tinyImageNet'], help='dataset')
    ...
    parser.add_argument('--VPbaseline', type=int, default=True, choices=[True, False])
    ...
    parser.add_argument('--testdata', type=str,  nargs='+',default=None)
    args = parser.parse_args()

    args.filename = '{}_{}_{}_{}_lr-{}_decay-{}_bsz-{}_warmup-{}_trial-{}'. \
        format(args.Method,args.dataset, args.model, args.arch, args.learning_rate, args.weight_decay, args.batch_size, args.warmup, args.trial)
    return args
...
def main():
    ...
    val_loader_list = [DataLoader(each,
                                  ...
                                  shuffle=False, sampler=val_sampler) for each in
                       val_dataset_list]
...
def train(train_loader, texts, model,frozen_model, prompter, add_prompter,
    ...
    for i, (images, target) in enumerate(tqdm(train_loader)):
        ...
        with autocast():
            ...
            output, text_logits , _= multiGPU_CLIP(model, prompted_images, text_tokens, target, device, prompt_token)
            CrossEntropyLoss = torch.nn.CrossEntropyLoss().to(device)
            loss_img = CrossEntropyLoss(output[:,target], ground_truth)#  11.154
            loss_txt = CrossEntropyLoss(text_logits[target,:], ground_truth)
        ...
        for i, (images, target) in enumerate(tqdm(val_loader)):
            ...
            with autocast():
                ...
                with torch.no_grad():
                    ...
                    CrossEntropyLoss = torch.nn.CrossEntropyLoss().to(device)
                    loss_img = CrossEntropyLoss(output_org_adv[:,target], ground_truth)
                    loss_txt = CrossEntropyLoss(logits_text[target,:], ground_truth)
                    loss = (loss_img + loss_txt)/2
                    losses.update(loss.item(), images.size(0))
